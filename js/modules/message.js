...
function initRealtimeForCurrentPatient() {
    ...
    window.Realtime.subscribe(AppState.currentPatient, async (event, newMessage) => {
        ...
        if (isOwnMessage) {
            console.log("📨 Message envoyé par moi-même - ignoré pour le badge");
        }
...
async function sendDocumentMessage() {
    ...
    Swal.fire({ title: "Envoi du document...", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        ...
        <div class="chat-container">
            <div class="chat-header">
                ...
                <div class="chat-header-actions">
                    <button id="attach-doc-btn" title="Pièce jointe"><i class="fa-solid fa-paperclip"></i></button>
                </div>
            </div>
...
function renderDocCard(msg) {
    ...
    if (extension === 'pdf') {
        iconClass = 'fa-file-pdf'; iconColor = 'text-red-500'; bgColor = 'bg-red-50'; fileType = 'PDF';
    } else if (extension === 'doc' || extension === 'docx') {
        iconClass = 'fa-file-word'; iconColor = 'text-blue-500'; bgColor = 'bg-blue-50'; fileType = 'DOC';
    } else if (extension === 'jpg' || extension === 'jpeg' || extension === 'png' || extension === 'gif' || extension === 'webp') {
        iconClass = 'fa-file-image'; iconColor = 'text-green-500'; bgColor = 'bg-green-50'; fileType = 'IMAGE';
    } else {
        iconClass = 'fa-file'; iconColor = 'text-slate-500'; bgColor = 'bg-slate-50'; fileType = 'FICHIER';
    }
    
    const isImage = ['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(extension);
    const previewHtml = isImage ? `<div class="mt-3"><img src="${msg.content}" class="w-full max-h-48 object-cover rounded-xl cursor-pointer border border-slate-200" onclick="window.open('${msg.content}')"></div>` : '';
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-all duration-200 mb-3 document-card">
            <div class="p-4">
                <div class="flex items-start gap-3">
                    ...
                    <button onclick="window.open('${msg.content}')" class="w-10 h-10 rounded-xl bg-slate-800 hover:bg-slate-700 text-white flex items-center justify-center transition-all active:scale-95 shrink-0" title="Télécharger"><i class="fa-solid fa-download text-sm"></i></button>
                </div>
                ${previewHtml}
            </div>
        </div>
...
// ============================================================
// FONCTIONS DE DÉTECTION DE FICHIERS
// ============================================================
...
function renderDocumentCard(url, filename) {
    ...
    return `
        <div class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 bg-white shadow-sm active:scale-98 transition-all cursor-pointer" onclick="window.open('${url}', '_blank')">
            <div class="w-10 h-10 rounded-xl ${fileInfo.bg} flex items-center justify-center"><i class="fa-solid ${fileInfo.icon} ${fileInfo.color} text-lg"></i></div>
            <div class="flex-1 min-w-0"><p class="text-xs font-semibold text-slate-800 truncate">${escapeHtml(shortName)}</p><p class="text-[9px] text-slate-400">${fileInfo.label} • Cliquer pour ouvrir</p></div>
...
// ============================================================
// RENDU DES CARTES STORY (MESSAGES)
// ============================================================
...
function renderStoryCard(msg, isReply = false) {
    ...
    if (msg.reply_to_id && !isOwnMessage) {
        ...
        if (parentMsg) {
            ...
            parentMessageHtml = `<div class="text-[9px] text-amber-600 mb-1 flex items-center gap-1"><i class="fa-solid fa-reply-all text-[8px]"></i><span class="truncate">↳ ${escapeHtml(parentMsg.sender_name || 'message')}: ${escapeHtml(parentContent)}</span></div>`;
        }
    }
        ...
        return `
            <div class="flex justify-end mb-2 ${isReply ? 'ml-8' : ''} ${tempClass} animate-fadeIn" data-message-id="${msg.id}">
                <div class="max-w-[75%] sm:max-w-[65%]">
                    ${fileUrl ? (isImage ? `<img src="${fileUrl}" class="rounded-2xl max-w-[200px] max-h-48 object-cover cursor-pointer mb-1" onclick="window.open('${fileUrl}')" loading="lazy" onerror="this.onerror=null; this.src='https://placehold.co/400x300?text=Image+non+chargée'">` : renderDocumentCard(fileUrl, msg.titre_media)) : ''}
                    ${content ? `<div class="chat-message-sent" style="background: var(--role-primary); border-bottom-right-radius: 4px;"><p class="text-white text-sm break-words">${escapeHtml(content)} ${humeurBadge}</p></div>` : ''}
                    <div class="flex justify-end items-center gap-1 mt-0.5"><span class="text-[9px] text-slate-400">${timeStr}</span><span class="message-status">${statusIcon}</span></div>
                </div>
            </div>
        `;
    }
    ...
    return `
        <div class="flex items-start gap-2 mb-2 ${isReply ? 'ml-8' : ''} ${tempClass} animate-fadeIn" data-message-id="${msg.id}">
            ...
            <div class="max-w-[75%] sm:max-w-[65%]">
                <div class="flex items-center gap-1 mb-0.5 flex-wrap"><span class="font-semibold text-slate-700 text-xs">${escapeHtml(msg.sender_name || 'Inconnu')}</span>${roleBadge}</div>
                ${parentMessageHtml}
                ${fileUrl ? (isImage ? `<div class="relative rounded-xl overflow-hidden mb-1 max-w-[200px]"><img src="${fileUrl}" class="rounded-xl max-h-48 object-cover cursor-pointer w-full" onclick="window.open('${fileUrl}')" loading="lazy" onerror="this.onerror=null; this.src='https://placehold.co/400x300?text=Image+non+chargée'"><div class="absolute bottom-2 right-2 bg-black/50 backdrop-blur px-1.5 py-0.5 rounded-lg"><i class="fa-regular fa-image text-white text-[8px]"></i></div></div>` : renderDocumentCard(fileUrl, msg.titre_media)) : ''}
                ${content ? `<div class="chat-message-received" style="background: #F1F5F9; border-bottom-left-radius: 4px;"><p class="text-slate-700 text-sm break-words">${escapeHtml(content)} ${humeurBadge}</p></div>` : ''}
                <div class="flex items-center gap-2 mt-0.5">
                    <span class="text-[9px] text-slate-400">${timeStr}</span>
                    <button onclick="window.replyToMessage('${msg.id}', '${escapeHtml(msg.sender_name || "l'utilisateur")}')" class="text-[9px] text-slate-400 hover:text-amber-500 transition"><i class="fa-solid fa-reply text-[8px]"></i></button>
                    <button onclick="window.showEmojiPickerForMessage('${msg.id}', this)" class="text-[9px] text-slate-400 hover:text-amber-500 transition"><i class="fa-regular fa-face-smile"></i></button>
                    ${isAidant && msg.id && !msg.is_temp ? `<button onclick="window.reportIssue('${msg.id}')" class="text-[9px] text-slate-400 hover:text-rose-500 transition"><i class="fa-regular fa-flag"></i></button>` : ''}
                </div>
                ${Object.keys(msg.reactions || {}).length > 0 ? `<div class="flex gap-1 mt-1">${Object.entries(msg.reactions || {}).map(([emoji, count]) => `<button onclick="window.sendReaction('${msg.id}', '${emoji}')" class="flex items-center gap-0.5 px-1.5 py-0.5 bg-slate-100 hover:bg-slate-200 rounded-full text-xs transition"><span class="text-sm">${emoji}</span><span class="text-[9px] font-medium text-slate-500">${count}</span></button>`).join('')}</div>` : ''}
                ${fileUrl && isImage && isAidant && !msg.is_temp ? `<div class="mt-0.5"><span class="text-[7px] text-slate-400"><i class="fa-regular fa-camera"></i> Photo</span></div>` : ''}
            </div>
        </div>
    `;
}
...
async function sendPhotoMessage() {
    ...
    Swal.fire({ title: "Envoi de la photo...", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
    ...
    try {
        ...
        if (currentReplyTo) formData.append('reply_to_id', currentReplyTo);
        if (currentReplyToName) formData.append('caption', "Réponse à " + currentReplyToName);
...
window.filterFeed = (type) => {
    ...
    if (type !== 'STORY') {
        cleanupRealtime();
    } else if (AppState.currentPatient) {
        initRealtimeForCurrentPatient();
    }
...
window.sendQuickMessage = async () => {
    ...
    try {
        ...
        if (activeTab === 'STORY') {
            ...
            container.insertAdjacentHTML('beforeend', renderStoryCard(tempMessage, false));
            scrollToBottom();
        }
        ...
        const body = { patient_id: AppState.currentPatient, content: content, is_photo: false, type_media: 'STORY' };
...
window.sendReaction = async (msgId, type) => {
    try {
        UI.vibrate();
        await secureFetch('/messages/react', { method: 'POST', body: JSON.stringify({ message_id: msgId, reaction_type: type }) });
        await loadFeed();
    } catch (err) {
        console.error(err);
...
window.appendMessagesToFeed = (newMessages) => {
    ...
    const newMessagesHtml = newMessages.map(msg => activeTab === 'DOCUMENT' ? renderDocCard(msg) : renderStoryCard(msg, false)).join('');
...
function initScrollDetection() {
    ...
    mainContent.addEventListener('scroll', () => {
        ...
        if (isBottom) { hideNewMessageBadge(); unreadMessagesCount = 0; }
    });
}
