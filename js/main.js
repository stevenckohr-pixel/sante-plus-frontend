...
// ============================================================
// PROMOTION DE L'INSTALLATION PWA
// ============================================================

let deferredPrompt = null;
let installationBannerShown = false;
...
// Afficher une bannière de rappel
function showInstallReminder() {
    ...
    // Créer la bannière de rappel
    const reminderBanner = document.createElement('div');
    reminderBanner.id = 'install-reminder';
    reminderBanner.innerHTML = `
        <div class="fixed bottom-24 left-4 right-4 bg-white rounded-2xl shadow-2xl border border-emerald-100 p-4 z-[10000] animate-slideUp">
            <div class="flex items-center gap-3">
                <div class="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center">
                    <i class="fa-solid fa-download text-emerald-600 text-xl"></i>
                </div>
                <div class="flex-1">
                    <p class="font-black text-slate-800 text-sm">Installer l'application</p>
                    <p class="text-xs text-slate-500">Accédez plus vite à Santé Plus</p>
                </div>
                <button id="install-now-btn" class="bg-emerald-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase">
                    Installer
                </button>
                <button id="close-reminder-btn" class="w-8 h-8 rounded-full flex items-center justify-center text-slate-400">
    ...
    // Animation d'entrée
    setTimeout(() => reminderBanner.classList.add('show'), 100);
    ...
    document.getElementById('close-reminder-btn')?.addEventListener('click', () => {
        reminderBanner.remove();
        localStorage.setItem('pwa_reminder_dismissed', Date.now().toString());
    });
}

// Afficher la bannière de première installation
function showInstallBanner() {
    if (installationBannerShown) return;
    if (isAppInstalled()) return;
    if (!deferredPrompt) return;
    
    installationBannerShown = true;
    
    Swal.fire({
        title: '<span class="text-xl font-black">📱 Installer l\'application</span>',
        html: `
            <div class="text-center">
                <div class="w-20 h-20 mx-auto bg-emerald-100 rounded-2xl flex items-center justify-center mb-4">
                    <i class="fa-solid fa-download text-emerald-600 text-3xl"></i>
                </div>
                <p class="text-slate-600 text-sm">Installez Santé Plus sur votre téléphone pour un accès plus rapide et une meilleure expérience.</p>
                <div class="mt-4 p-3 bg-slate-50 rounded-xl text-left text-xs text-slate-500">
                    <p><i class="fa-solid fa-rocket mr-2"></i> Lancement plus rapide</p>
                    <p class="mt-1"><i class="fa-solid fa-wifi mr-2"></i> Fonctionne hors ligne (partiellement)</p>
                    <p class="mt-1"><i class="fa-solid fa-bell mr-2"></i> Notifications push</p>
                </div>
            </div>
        `,
        icon: 'info',
        ...
        customClass: {
            popup: 'rounded-2xl',
            confirmButton: 'rounded-xl px-5 py-2.5 text-[10px] font-black uppercase tracking-wider',
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    const hasSeenInstallBanner = localStorage.getItem('pwa_banner_seen');
    const reminderDismissed = localStorage.getItem('pwa_reminder_dismissed');
    const daysSinceDismissed = reminderDismissed 
        ? (Date.now() - parseInt(reminderDismissed)) / (1000 * 60 * 60 * 24)
        : 7;
    
    if (!hasSeenInstallBanner) {
        // Première visite : afficher la bannière après 2 secondes
        setTimeout(() => {
            showInstallBanner();
            localStorage.setItem('pwa_banner_seen', 'true');
        }, 2000);
    } else if (!reminderDismissed || daysSinceDismissed >= 3) {
        // Visiteur qui revient : afficher le rappel après 5 secondes
        setTimeout(() => {
            showInstallReminder();
        }, 5000);
    }
...
window.addEventListener('appinstalled', () => {
    ...
    document.getElementById('install-reminder')?.remove();
});
